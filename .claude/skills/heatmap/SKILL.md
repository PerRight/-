...
2. 히트맵을 렌더링한다:
   ...
   ```
3. 두 출력물이 생성됐는지 확인한다:
   - `output/heatmap_3d.html` — 인터랙티브 (회전·확대·호버)
   - `output/heatmap_3d.png` — 보고서용 정적 이미지
4. PNG를 Read 도구로 열어 렌더링이 정상인지(핫스팟·한글 레이블) 육안 확인한 뒤,
   인터랙티브 버전을 기본 브라우저로 연다:
   ```powershell
   Start-Process (Resolve-Path output\heatmap_3d.html)
   ```
5. 셀 수·탁도 범위를 요약해 보고한다.
...
- 실측 데이터가 생기면 같은 컬럼 형식으로 `data/measurements.csv`만 교체하면 된다
  (컬럼: run_id, timestamp, x_m, y_m, depth_m, turbidity_ntu).
